...
public class NewMapGen : BoardCreator
{
    private IntRange quaterRandom = new IntRange(1, 4);
    ...
    private void BoardSetup()
    {
        ...
        Vector3 playerPos = new Vector3(100, 70, 0);
        GameManager.instance.GetPlayer().transform.position = playerPos;
        Vector3 exitPos = new Vector3(101, 70, 0);
    ...
    {
        IntRange playerRange = new IntRange(1, floorCount);
        int p = playerRange.Random;

        int e = p + floorCount / 2;
        if (e > floorCount)
            e = e - floorCount;
        ...
        for (int i = 0; i < columns+2; i++)
        {
            for (int j = 0; j < rows+2; j++)
            {
                ...
                if (map[i, j] == 0)
                {
                    ...
                    if(tmp == e)
                    {
                        Vector3 exitPos = new Vector3(i, j, 0);
                        GameObject tileInstance = Instantiate(exit, exitPos, Quaternion.identity) as GameObject;
                        tileInstance.transform.parent = boardHolder.transform;
                        continue;
                    }
